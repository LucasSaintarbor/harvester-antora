...
:keywords: ["Harvester", "harvester", "Rancher", "rancher", "Install Harvester", "Installing Harvester", "Harvester Installation", "PXE Boot Install"]
...
To see sample iPXE scripts, please visit https://github.com/harvester/ipxe-examples[Harvester iPXE Examples].
...
[IMPORTANT]
====
The installer automatically checks if the hardware meets the link:./requirements/#hardware-requirements[minimum requirements] for production use. If any of the checks fail, installation will be stopped. To override this behavior, set either the link:./harvester-configuration/#installskipchecks[configuration file option] `install.skipchecks=true` or the <<harvesterinstallskipcheckstrue,kernel parameter>> `harvester.install.skipchecks=true`.
====
...
* Download the required files from the https://github.com/harvester/harvester/releases[Harvester releases page].
...
Harvester v1.3.0 introduces roles that you can assign to nodes to support different scenarios. For more information, see link:harvester-configuration#installrole[Harvester Configuration].
...
Create a xref:./harvester-configuration.adoc[Harvester configuration file] called `config-create.yaml` for `CREATE` mode. Modify the values as needed:
...
Create a xref:./harvester-configuration.adoc[Harvester configuration file] called `config-join.yaml` for `JOIN` mode. Modify the values as needed:
...
In the PXE installation scenario, you are required to add the _routers_ option (`option routers`) when configuring the DHCP server. This option is used to add the default route on the Harvester host. Without the default route, the node will fail to start.
...
For more information about Harvester configuration, please refer to the xref:./harvester-configuration.adoc[Harvester configuration] page.
...
If you want to promote management nodes from different zones, you can add the node label `topology.kubernetes.io/zone` in the link:./harvester-configuration.adoc#oslabels[os.labels] config. In this case, at least three different zones are required.
...
The installer does not preload images during installation and instead pulls all required images from the internet after installation is completed. Usage of this parameter is not recommended in most cases. For more information, see xref:./net-install.adoc[Net Install ISO].
