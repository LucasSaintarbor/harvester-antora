...
In some environments, you configure link:../airgap.adoc#configure-an-http-proxy-during-installation[http-proxy] of link:../install/harvester-configuration.adoc#osenvironment[OS Environment] during Harvester installation.

==== Configure HTTP Proxy After First Node is Ready

After the first node is installed successfully, you login into the `Harvester GUI` to configure link:../airgap.adoc#configure-an-http-proxy-in-harvester-settings[http-proxy] of link:../install/harvester-configuration.adoc#system_settings[Harvester System Settings].
...
When the nodes in the cluster do not use the HTTP Proxy to communicate with each other, after the first node is installed successfully, you need to configure link:../airgap.adoc#configure-an-http-proxy-in-harvester-settings[http-proxy.noProxy] against the CIDR used by those nodes.
...
When a Harvester configuration file is used in ISO installation, please configure proper `http-proxy` in link:../install/harvester-configuration.adoc#system_settings[Harvester System Settings].

=== PXE Boot Installation

When xref:../install/pxe-boot-install.adoc[PXE Boot Installation] is adopted, please configure proper `http-proxy` in link:../install/harvester-configuration.adoc#osenvironment[OS Environment] and link:../install/harvester-configuration.adoc#system_settings[Harvester System Settings].
...
* Click the `Support` link at the bottom-left of Harvester Web UI.
  image:/img/v1.2/troubleshooting/harvester-sb-support-link.png[]
* Click `Generate Support Bundle` button.
  image:/img/v1.2/troubleshooting/harvester-sb-support-button.png[]
* Enter a useful description for the support bundle and click `Create` to generate and download a support bundle.
  image:/img/v1.2/troubleshooting/harvester-sb-support-modal.png[]

[NOTE]
====

Whenever you encounter an issue that may be related to workloads deployed in custom namespaces, configure the link:../advanced/settings.adoc#support-bundle-namespaces[support-bundle-namespaces] setting to include those namespaces as data sources. The support bundle only collects data from the configured namespaces.

For timeout errors, you can adjust the value of the link:../advanced/settings.adoc#support-bundle-timeout[support-bundle-timeout] setting and then restart the support bundle generation process.

If you intend to use a non-default container image, you can configure the link:../advanced/settings.adoc#support-bundle-image[support-bundle-image] setting.
====
 ...
 ** link:../advanced/index#support-bundle-expiration[support-bundle-expiration]: Defines the time allowed for retaining a support bundle file
 ** link:../advanced/index#support-bundle-timeout[support-bundle-timeout]: Defines the time allowed for generating a support bundle file
...
image::/img/v1.2/troubleshooting/support-access-embedded-ui.png[]
...
If you changed
link:../advanced/settings.adoc#ssl-parameters[SSL/TLS enabled protocols and ciphers settings]
