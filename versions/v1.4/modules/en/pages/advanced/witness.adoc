...
Harvester v1.3.0 supports clusters with two management nodes and one witness node (and optionally, one or more worker nodes). For more information about node roles in Harvester, see link:../host/host.adoc#role-management[Role Management].
...
image:/img/v1.3/advanced/new-storageclass-rep-2.png[new storageclass replica 2]
image:/img/v1.3/advanced/set-to-default-sc.png[set to default]

If you already created volumes using the original default StorageClass, you can modify the replica count on the *Volume* screen of the link:../troubleshooting/harvester/#access-embedded-rancher-and-longhorn-dashboards[embedded Longhorn UI].

image:/img/v1.3/advanced/redirect-to-longhorn-vol-page.png[redirect-to-longhorn-volume-page]
image:/img/v1.3/advanced/update-replica-2.png[update-replica-count-to-2]
...
image:/img/v1.3/advanced/create-policy-with-all-nodes.png[create network config with all nodes]
  image:/img/v1.3/advanced/no-uplink.png[no uplink]

The workaround is to select a non-witness node and then select a NIC that can be used with that specific node.

image:/img/v1.3/advanced/create-policy-with-specific-node.png[create network config with specific node]
  image:/img/v1.3/advanced/get-uplink.png[get uplink]
...
image::/img/v1.3/advanced/vm-migration-witness-node.png[vm migration target witness node]
