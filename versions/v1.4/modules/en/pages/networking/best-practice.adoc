...
:keywords: ["Harvester", "Networking", "Best Practice"]
...
. On the link:../vm/access-to-the-vm.adoc#access-with-the-harvester-ui[*Virtual Machines* screen of the Harvester UI], verify that the status of all VMs is either _Running_ or _Stopped_.
. On the link:../troubleshooting/harvester.adoc#access-embedded-rancher-and-longhorn-dashboards[embedded Longhorn dashboard], verify that the status of all Longhorn volumes is _Healthy_.
. (Optional) On the *Harvester Support* screen, generate a link:../troubleshooting/harvester.adoc#generate-a-support-bundle[support bundle] for comparison purposes.
...
. link:../host/host.adoc#node-maintenance[Enable maintenance mode] on the target node to automatically migrate all VMs to other nodes.
...
There are one or more link:./clusternetwork.adoc#how-to-create-a-new-cluster-network[Network Config] under every link:./clusternetwork.adoc#cluster-network[Cluster Network] on Harvester. Each `Network Config` is backed by a `VlanConfig` CRD object.
...
+
During system maintenance, you can modify the https://longhorn.io/docs/1.4.3/references/settings/#replica-replenishment-wait-interval[`replica-replenishment-wait-interval`] value using the link:../troubleshooting/harvester.adoc#access-embedded-rancher-and-longhorn-dashboards[embedded Longhorn UI] to enable faster replica rebuilding.
+
...
. Replace the NICs.
. Restart the node.
. <<collect-information,Collect information>> about the current network configuration and status.

If you observe any abnormalities, generate a link:../troubleshooting/harvester.adoc#generate-a-support-bundle[support bundle] for troubleshooting purposes.
...
. (Optional) Manually link:../vm/live-migration.adoc#starting-a-migration[migrate VMs] to this node.
