...
:id: index
:keywords: ["Harvester", "harvester", "Rancher", "rancher", "ISO Installation"]
:sidebar_label: ISO Installation
:sidebar_position: 2

Harvester ships as a bootable appliance image, you can install it directly on a bare metal server with the ISO image. To get the ISO image, download *💿 harvester-v1.x.x-amd64.iso* from the https://github.com/harvester/harvester/releases[Harvester releases] page.
...
The following https://youtu.be/X0VIGZ_lExQ[video] shows a quick overview of an ISO installation.+++<div class="text-center">++++++<iframe width="800" height="400" src="https://www.youtube.com/embed/X0VIGZ_lExQ" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen="">++++++</iframe>++++++</div>+++

== Installation Steps

. Mount the Harvester ISO file and boot the server by selecting the `Harvester Installer` option.
+
image::/img/v1.2/install/iso-install.png[iso-install.png]
+
...
+
image::/img/v1.3/install/hardware-checks.png[hardware-checks.png]

. Use the arrow keys to choose an installation mode. By default, the first node will be the management node of the cluster.
+
image::/img/v1.2/install/choose-installation-mode.png[choose-installation-mode.png]
...
[IMPORTANT]
====
 When there are 3 nodes, the other 2 nodes added first are automatically promoted to management nodes to form an HA cluster. If you want to promote management nodes from different zones, you can add the node label `topology.kubernetes.io/zone` in the link:./harvester-configuration.adoc#oslabels[os.labels] config by providing a URL of xref:./harvester-configuration.adoc[Harvester configuration] on the customize the host step. In this case, at least three different zones are required.
====
...
+
image::/img/v1.3/install/select-role.png[choose-node-role.png]
...
. Choose the installation disk you want to install the Harvester cluster on and the data disk you want to store VM data on. By default, Harvester uses https://en.wikipedia.org/wiki/GUID_Partition_Table[GUID Partition Table (GPT)] partitioning schema for both UEFI and BIOS. If you use the BIOS boot, then you will have the option to select https://en.wikipedia.org/wiki/Master_boot_record[Master boot record (MBR)].
+
image::/img/v1.2/install/choose-installation-target-data-disk.png[choose-installation-target-data-disk.png]
...
+
image::/img/v1.2/install/config-hostname.png[config-hostname.png]

. Configure network interface(s) for the management network. By default, Harvester creates a link:./requirements.adoc#hardware-requirements[bonded NIC] named `mgmt-bo` for the link:../networking/clusternetwork.adoc#built-in-cluster-network[built-in management cluster network], and the IP address can be configured via DHCP or statically assigned.
+
image::/img/v1.2/install/config-network.png[config-network.png]
+
...
+
In addition, you are required to add the _routers_ option (`option routers`) when configuring the DHCP server. This option is used to add the default route on the Harvester host. Without the default route, the node will fail to start.
 For example:
+
----
 Linux~ # ip route
...
----
+
For more information, see link:./pxe-boot-install.adoc#dhcp-server-configuration[DHCP Server Configuration].
====
...
+
image::/img/v1.2/install/config-dns-server.png[config-dns-server.png]
...
+
image::/img/v1.2/install/config-virtual-ip.png[config-virtual-ip.png]

. Configure the `Cluster token`. This token is used for adding other nodes to the cluster.
+
image::/img/v1.2/install/config-cluster-token.png[config-cluster-token.png]

. Configure and confirm a `Password` to access the node. The default SSH user is `rancher`.
+
image::/img/v1.2/install/config-password.png[config-password.png]
...
+
image::/img/v1.2/install/config-ntp-server.png[config-ntp-server.png]

. (Optional) If you need to use an HTTP proxy to access the outside world, enter the `Proxy address`. Otherwise, leave this blank.
+
image::/img/v1.2/install/config-proxy.png[config-proxy.png]
...
+
image::/img/v1.2/install/import-ssh-keys.png[import-ssh-keys.png]

. (Optional) If you need to customize the host with a xref:./harvester-configuration.adoc[Harvester configuration] file, enter the `HTTP URL` here.
+
image::/img/v1.2/install/remote-config.png[remote-config.png]

. Review and confirm your installation options. After confirming the installation options, Harvester will be installed to your host. The installation may take a few minutes to be complete.
+
image::/img/v1.2/install/confirm-install.png[confirm-install.png]

. Once the installation is complete, your node restarts. After the restart, the Harvester console displays the management URL and status. The default URL of the web interface is `+https://your-virtual-ip+`. You can use `F12` to switch from the Harvester console to the Shell and type `exit` to go back to the Harvester console.
+
...
+
image::/img/v1.2/install/iso-installed.png[iso-installed.png]

. You will be prompted to set the password for the default `admin` user when logging in for the first time.
+
image::/img/v1.2/install/first-time-login.png[first-login.png]
...
image::/img/v1.2/install/invalid-dimensions.png[invalid-dimensions.png]
...
image::/img/v1.2/install/force-resolution.png[force-resolution.png]
...
+
image::/img/v1.2/install/grub-menu.png[grub-menu.png]

. Append `vga=792` to the line started with `$linux`:
+
image::/img/v1.2/install/edit-menu-entry.png[edit-menu-entry.png]
...
image::/img/v1.3/install/join-node-not-ready.png[Joining nodes stuck at the "NotReady" state]
...
image::/img/v1.3/install/ssl-certificates-setting.png[The SSL certificate setting]
